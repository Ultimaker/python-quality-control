...
HARD_LIMIT = 400
BASELINE_PATH = Path.cwd() / "file_budget_baseline.json"
SOURCE_SUFFIXES = (".py", ".cpp", ".h", ".qml", ".sh")
...
def main(argv: list) -> int:
    ...
    for name in argv:
        path = Path(name)
        if "addons" in path.parts or ".git" in path.parts or "ci" in path.parts:
            continue
        ...
        if lines > allowed:
            if name in baseline:
                ...
                failures.append(
                    f"{name}: {lines} lines exceeds the {HARD_LIMIT}-line budget (target 300, AGENTS.md). "
